...
parser.add_argument('--labeled_sample_list', default='sunrgbd_v1_train_0.05.txt',
                    help='Labeled sample list from a certain percentage of training [static]')
parser.add_argument('--detector_checkpoint', default='ckpts/checkpoint_400.tar')
parser.add_argument('--log_dir', default='eval_sunrgbd', help='Dump dir to save model checkpoint')
...
parser.add_argument('--eval_interval', type=int, default=200, help='epoch interval to evaluate model')
parser.add_argument('--save_interval', type=int, default=200, help='epoch interval to save model')
parser.add_argument('--secoitn_size', type=int, default=50, help='epoch interval to evaluate model')
...
if FLAGS.dataset == 'sunrgbd':
    ...
    Validate_DATASET = ScannetSSLUnlabeledDataset(labeled_sample_list=FLAGS.labeled_sample_list,
                                                   num_points=NUM_POINT,
                                                   use_color=FLAGS.use_color,
                                                   use_height=(not FLAGS.no_height),
                                                   load_labels=FLAGS.view_stats,
...
# RESAMPLE_LABELED_DATALOADER = DataLoader(LABELED_DATASET, batch_size=batch_size_list[0], sampler = ImbalancedDatasetSampler(LABELED_DATASET),
#                                num_workers=batch_size_list[0], worker_init_fn=my_worker_init_fn)
...
Valid_DATALOADER = DataLoader(Validate_DATASET, batch_size=batch_size_list[1],
                              shuffle=False, num_workers=batch_size_list[1]//2, worker_init_fn=my_worker_init_fn)
...
def train_one_epoch(global_step):
    ...
    for batch_idx, batch_data_label in enumerate(LABELED_DATALOADER):
        ...
        end_points['current_epoch'] = EPOCH_CNT
        end_points['section_size'] = FLAGS.secoitn_size
    ...
    for batch_idx, batch_data_label in enumerate(Valid_DATALOADER):
        ...
        valid_min_class = np.zeros(pred_sem_cls.shape[0])
            ...
            if current_index[0].shape[0] == 0:
                ...
                for index in current_index[0]:
                    ...
                    if max_sem_prob[i, index] < current_min_confid:
                        ...
                        current_min_label = current_label
                valid_min_confid[i] = current_min_confid
                valid_min_class[i] = current_min_label
        test_min_confid.append(valid_min_confid)
        test_min_label.append(valid_min_class)
    ...
    test_min_label = np.concatenate(test_min_label)

    dict_high_conf_index = Munch()

    for i in range(18):
        if dict_ave_validate_conf['%d_len' % i] == 0:
            dict_ave_validate_conf_mean['%d' % i] = 0.2
        else:
            dict_ave_validate_conf_mean['%d' %i] = dict_ave_validate_conf['%d' % i]/dict_ave_validate_conf['%d_len' % i]

    for current_label in range(18):
        label_pos = np.where(test_min_label == current_label)

        if dict_ave_validate_conf_mean['%d' % current_label] > 0.8:
            high_conf_label_pos = label_pos[0][
                np.where(test_min_confid[label_pos] > dict_ave_validate_conf_mean['%d' % current_label])]
            dict_high_conf_index['%d' % current_label] = high_conf_label_pos.tolist()

        elif dict_ave_validate_conf_mean['%d' % current_label] < 0.8:
            low_conf_label_pos = label_pos[0][np.where(
                (test_min_confid[label_pos] > dict_ave_validate_conf_mean['%d' % current_label]))]
    ...
    f = open("dict_minconf.txt", "w")
    js = json.dumps(test_min_confid.tolist())
    f.write(js)
    f.close()

    f = open("current_epoch.txt", "w")
    js = json.dumps(EPOCH_CNT)
    f.write(js)
    f.close()
...
#         if dict_ave_validate_conf_mean['%d' % current_label] > 0.7:
#             high_conf_label_pos = label_pos[0][
#                 np.where(test_min_confid[label_pos] > dict_ave_validate_conf_mean['%d' % current_label])]
#             dict_high_conf_index['%d' % current_label] = high_conf_label_pos.tolist()

#         elif dict_ave_validate_conf_mean['%d' % current_label] < 0.7:
#             low_conf_label_pos = label_pos[0][np.where(
#                 (test_min_confid[label_pos] > dict_ave_validate_conf_mean['%d' % current_label]))]
#             dict_high_conf_index['%d_low' % current_label] = low_conf_label_pos.tolist()
...
#     f.close()
    ...
    global LABELED_DATALOADER
    LABELED_DATALOADER = DataLoader(LABELED_DATASET, batch_size=batch_size_list[0],
                                             sampler=ImbalancedDatasetSampler(LABELED_DATASET),
                                             num_workers=batch_size_list[0], worker_init_fn=my_worker_init_fn)
    global UNLABELED_DATALOADER
    UNLABELED_DATALOADER = DataLoader(UNLABELED_DATASET, batch_size=batch_size_list[1],
                                      sampler=Unlabeled_ImbalancedDatasetSampler(UNLABELED_DATASET), num_workers=batch_size_list[1] // 2,
...
def train():
    ...
    for epoch in range(start_from, MAX_EPOCH):
        ...
        if EPOCH_CNT % FLAGS.secoitn_size == 0 and EPOCH_CNT>0:
            print('**************************************************')
