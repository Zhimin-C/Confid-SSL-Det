...
BASE_DIR = '/home//scratch1link/Semi-Vit/det/scannet/3DIoUMatch/scannet'
ROOT_DIR = BASE_DIR
...
class ScannetDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', labeled_ratio=0.1, labeled_sample_list=None, num_points=20000,
        ...
        if split_set=='all':            
            ...
            split_filenames = os.path.join(ROOT_DIR, 'meta_data',
        ...
        # added
        self.raw_data_path = os.path.join(ROOT_DIR, 'meta_data')
    ...
    def get_labeled_samples(self):
        if self.labeled_sample_list is not None:
            labeled_scan_names = [x.strip() for x in open(
                os.path.join(ROOT_DIR, 'meta_data', self.labeled_sample_list)).readlines()]
        else:
            ...
            while True:
                ...
                if np.sum(class_mask) == DC.num_class:
                    ...
                    with open(os.path.join(ROOT_DIR, 'meta_data/scannetv2_train_{}.txt'.format(self.labeled_ratio)), 'w') as f:
