...
def get_pseudo_labels(end_points, ema_end_points, pred_center, pred_sem_cls, pred_objectness, pred_heading_scores,
    ...
    if end_points['current_epoch']==0:
        ...
        z.close()
        flex_threshold = torch.zeros(max_cls.shape, device = max_cls.device)
        ...
        for current_label in dict_avgconf:
            ...
            if dict_avgconf['%d' % current_label]/(2-dict_avgconf['%d' % current_label]) < 0.2:
                flex_threshold[argmax_cls == current_label] = 0.2
            elif dict_avgconf['%d' % current_label]/(2-dict_avgconf['%d' % current_label]) >0.8:
                flex_threshold[argmax_cls == current_label] = 0.8
            else:
                flex_threshold[argmax_cls == current_label] = dict_avgconf['%d' % current_label]/(2-dict_avgconf['%d' % current_label])
        cls_mask = max_cls.ge(flex_threshold).float()
